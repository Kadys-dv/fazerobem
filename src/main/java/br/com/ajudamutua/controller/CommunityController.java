...
public class CommunityController { private final CommunityService service; public CommunityController(CommunityService service){this.service=service;}
 ...
 @PostMapping("/contributions") @ResponseStatus(HttpStatus.CREATED) public LedgerEntry contribution(@Valid @RequestBody ApiDtos.Contribution in){return service.contribute(in);}
 @PostMapping("/aid-requests") @ResponseStatus(HttpStatus.CREATED) public AidRequest aid(@Valid @RequestBody ApiDtos.CreateAid in){return service.requestAid(in);}
 ...
 @PostMapping(value="/aid-requests/{id}/documents",consumes=MediaType.MULTIPART_FORM_DATA_VALUE) @ResponseStatus(HttpStatus.CREATED) public AidDocument document(@PathVariable UUID id,@RequestParam String documentType,@RequestPart("file") MultipartFile file){return service.addDocument(id,documentType,file);}
