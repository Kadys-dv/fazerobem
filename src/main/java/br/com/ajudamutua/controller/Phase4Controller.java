...
import br.com.ajudamutua.model.*; import br.com.ajudamutua.payment.PaymentService; import br.com.ajudamutua.privacy.PrivacyService; import jakarta.servlet.http.HttpServletRequest; import jakarta.validation.Valid; import jakarta.validation.constraints.*; import org.springframework.http.HttpStatus; import org.springframework.web.bind.annotation.*; import java.util.*;
@RestController @RequestMapping("/api/v1") public class Phase4Controller {
 ...
 @PostMapping("/privacy/private-data") public MemberPrivateData privateData(@Valid @RequestBody PrivateDataRequest in){return privacy.savePrivateData(in.cpf(),in.address());}
 @PostMapping("/privacy/consents") public ConsentRecord consent(@Valid @RequestBody ConsentRequest in,HttpServletRequest req){return privacy.consent(in.type(),in.version(),in.accepted(),req.getRemoteAddr());}
 ...
 @PostMapping("/kyc/{memberId}/review") public KycVerification review(@PathVariable UUID memberId,@Valid @RequestBody KycReview in){return privacy.review(memberId,in.status(),in.reason());}
