...
@RestController @RequestMapping("/api/v1") public class Phase4Controller {
 ...
 public record PrivateDataRequest(@NotBlank String cpf,String address){} public record ConsentRequest(@NotNull ConsentType type,@NotBlank String version,boolean accepted){} public record KycReview(@NotNull KycStatus status,String reason){} public record WebhookBody(@NotBlank String providerReference,@NotBlank String status){} public record ReconciliationNote(@NotBlank @Size(max=500) String note){}
 ...
 @PostMapping("/payments/attempts/{paymentId}/reconciliation-note") public PaymentAttempt reconciliationNote(@PathVariable UUID paymentId,@RequestBody ReconciliationNote in){return payments.acknowledgeReconciliation(paymentId,in.note());}
