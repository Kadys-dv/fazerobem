const state={me:null,policies:[],myRequests:[],installPrompt:null};
...
const fraudLabels={PENDING:'Aguardando triagem',CLEARED:'Triagem liberada',REVIEW_REQUIRED:'Revisão adicional necessária',BLOCKED:'Bloqueado para revisão'};
...
async function loadMine(){if(!state.me?.memberId)return;try{state.myRequests=await api('/api/v1/aid-requests/mine');$('myRequests').classList.toggle('empty-state',state.myRequests.length===0);$('myRequests').innerHTML=state.myRequests.length?state.myRequests.map(r=>`<div class="request-row"><div><strong>${escapeHtml(labels[r.category]||r.category)}</strong><span class="status-pill">${escapeHtml(statusLabels[r.status]||r.status)}</span></div><small>${brl(r.amount)} · ${new Date(r.createdAt).toLocaleDateString('pt-BR')}</small><small>${escapeHtml(r.reason)}</small><button class="ghost request-detail-btn" type="button" data-aid-id="${r.id}">Ver critérios e documentos</button></div>`).join(''):'Nenhum pedido enviado ainda.'}catch(error){toast('Não foi possível carregar seus pedidos: '+error.message)}}

async function openAidDetail(id){const request=state.myRequests.find(r=>r.id===id);if(!request)return;const dialog=$('aidDetailDialog');$('documentAidId').value=id;$('detailTitle').textContent=labels[request.category]||request.category;$('aidDetailSummary').innerHTML=`<div class="detail-card"><span>Valor solicitado</span><strong>${brl(request.amount)}</strong></div><div class="detail-card"><span>Status</span><strong>${escapeHtml(statusLabels[request.status]||request.status)}</strong></div><div class="detail-card"><span>Enviado em</span><strong>${new Date(request.createdAt).toLocaleDateString('pt-BR')}</strong></div>`;$('eligibilityBox').textContent='Carregando…';$('documentList').textContent='Carregando…';$('documentForm').classList.toggle('hidden',request.status!=='PENDING');dialog.showModal();try{const [eligibility,documents]=await Promise.all([api(`/api/v1/aid-requests/${id}/eligibility`),api(`/api/v1/aid-requests/${id}/documents`)]);renderEligibility(eligibility);renderDocuments(documents)}catch(error){$('eligibilityBox').innerHTML=`<div class="form-message error">${escapeHtml(error.message)}</div>`;$('documentList').innerHTML='<div class="empty-state">Não foi possível carregar os documentos.</div>'}}
function renderEligibility(e){const blockers=e.blockers||[];const warnings=e.warnings||[];const tone=e.eligible?'eligibility-ok':'eligibility-wait';$('eligibilityBox').innerHTML=`<div class="${tone}"><strong>${e.eligible?'Critérios atendidos neste momento':'Ainda existem pendências'}</strong><p>Limite da categoria: ${brl(e.categoryMaxAmount)} · limite atual do fundo: ${brl(e.currentFundLimit)}</p><p>Documentos enviados: ${e.documentCount}${e.documentRequired?' · documentação obrigatória':''}</p><p>Antifraude: ${escapeHtml(fraudLabels[e.fraudStatus]||e.fraudStatus||'Aguardando')}</p>${blockers.length?`<div class="criteria-list"><strong>O que falta</strong><ul>${blockers.map(x=>`<li>${escapeHtml(x)}</li>`).join('')}</ul></div>`:''}${warnings.length?`<div class="criteria-list warning-list"><strong>Avisos</strong><ul>${warnings.map(x=>`<li>${escapeHtml(x)}</li>`).join('')}</ul></div>`:''}</div>`}
function renderDocuments(documents){$('documentList').innerHTML=documents.length?documents.map(d=>`<div class="document-row"><div><strong>${escapeHtml(d.documentType.replaceAll('_',' '))}</strong><small>${escapeHtml(d.fileName)} · ${formatBytes(d.sizeBytes)} · ${new Date(d.createdAt).toLocaleDateString('pt-BR')}</small></div><a class="ghost link-button" href="/api/v1/aid-documents/${d.id}/content">Baixar</a></div>`).join(''):'<div class="empty-state">Nenhum documento enviado.</div>'}
...
$('closeAidDetail').addEventListener('click',()=>$('aidDetailDialog').close());
...
$('myRequests').addEventListener('click',event=>{const button=event.target.closest('.request-detail-btn');if(button)openAidDetail(button.dataset.aidId)});
...
$('contributionForm').addEventListener('submit',async event=>{event.preventDefault();try{if(!$('contributionConsent').checked)throw new Error('Confirme que compreendeu a natureza da contribuição.');const amount=parseMoney($('contributionAmount').value);await api('/api/v1/contributions',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({memberId:state.me.memberId,amount})});event.target.reset();document.dispatchEvent(new Event('member-contribution-recorded'));await loadPublic();toast('Contribuição sandbox registrada.')}catch(error){toast(error.message)}});
$('aidForm').addEventListener('submit',async event=>{event.preventDefault();try{const payload={memberId:state.me.memberId,amount:parseMoney($('aidAmount').value),category:$('aidCategory').value,reason:$('aidReason').value.trim(),emergency:$('aidEmergency').checked};if(!payload.category)throw new Error('Selecione uma categoria.');const created=await api('/api/v1/aid-requests',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});event.target.reset();renderCategoryOptions();toast('Pedido enviado para análise.');await Promise.all([loadMine(),loadPublic()]);await openAidDetail(created.id)}catch(error){toast(error.message)}});
$('documentForm').addEventListener('submit',async event=>{event.preventDefault();const file=$('documentFile').files[0];if(!file)return toast('Selecione um arquivo.');if(file.size>5*1024*1024)return toast('O arquivo deve ter no máximo 5 MB.');const allowed=['application/pdf','image/jpeg','image/png'];if(file.type&&!allowed.includes(file.type))return toast('Use PDF, JPG ou PNG.');const aidId=$('documentAidId').value;const form=new FormData();form.append('documentType',$('documentType').value);form.append('file',file);try{$('uploadDocumentBtn').disabled=true;await api(`/api/v1/aid-requests/${aidId}/documents`,{method:'POST',body:form});event.target.reset();toast('Documento enviado com segurança.');const [eligibility,documents]=await Promise.all([api(`/api/v1/aid-requests/${aidId}/eligibility`),api(`/api/v1/aid-requests/${aidId}/documents`)]);renderEligibility(eligibility);renderDocuments(documents)}catch(error){toast(error.message)}finally{$('uploadDocumentBtn').disabled=false}});
